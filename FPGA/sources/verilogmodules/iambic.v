...
module iambic #
(
    parameter clock_speed = 48              // default clock speed of 48kHz from PLL 
)
(
    input wire clock,
	input wire clken,                      // clock enable					
	input wire [5:0] cw_speed,             // 1 to 60 WPM
	input wire iambic,                     // 0 = straight/bug,  1 = Iambic 
	input wire keyer_mode,                 // 0 = Mode A, 1 = Mode B
	input wire [7:0] weight,               // 33 to 66, nominal is 50
	input wire letter_space,               // 0 = off, 1 = on
	input wire dot_key,                    // dot paddle  input, active high
...
//
// will be clocked at 122.88MHz but with a 48KHz clock enable
//
...
LOOP:
	begin
		if(!iambic) 							// Straight/External key or bug
		begin
...
DOTHELD:
	begin
		if (dot) 										// dot has been set during the dash so service
			...
			key_state <= PREDASH;
		else if (letter_space) 				// Letter space enabled so clear any pending dots or dashes
		begin 
        	dot_memory  <= 0;
        	dash_memory <= 0;
